...
server._init_master_server = function (options, port) {
  ...
  var default_router = server_config.routes.filter((route) => {
    return route.default
  })[0] || {}

  var dev_index = default_router.location || '/s'
  var default_root = default_router.root && default_router.root[0]
  var dev_root = default_root
    ? node_path.join(default_root, dev_index)
    : options.dist
...
server._delegate = function (options, callback) {
  ...
  request({
    ...
    if (!err && res.statusCode === 200 && json.code === 200) {
      logger.info('{{green success!}}')
      return callback(null)
    }

    callback(json && json.message || 'fails to delegate to master server.')
  })
}
...
server.reverse_route = function (file, options) {
  return reverse_router.route(file, options.config.server.routes)
}
